...
int main(int argc, char* argv[]) {
	if (argc < 8) {
		printf("%s verbose offset1 offset2 nullpos1 nullpos2 n iterations function\n", argv[0]);
		return 1;
	}

	int v    = atoi(argv[1]);

	int ofs1 = 0;//atoi(argv[2]);
	int ofs2 = 0;//atoi(argv[3]);
	...
	if (v) {
		...
		printf("string 2 at %d, length %d, limit %d: ", ofs2, len2-ofs2, n);
		if (fun == 0)
	...
	switch (fun) {
		case 0:
			TIMEIT(l1 = strncmp(text1, text2, n));
			break;
		case 1:
			TIMEIT(l2 = sse2_strncmp_unsafe(text1, text2, n));
			break;
		default:
			l1 = strncmp(text1, text2, n);
			l2 = sse2_strncmp_unsafe(text1, text2, n);
			if (sign(l1) != sign(l2)) {
